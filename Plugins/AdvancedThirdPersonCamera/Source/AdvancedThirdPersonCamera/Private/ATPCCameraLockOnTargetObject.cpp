...
void UATPCCameraLockOnTargetObject::Tick(float DeltaSeconds)
{
	...
	if (TargetActor.IsValid() && locationObject != nullptr && (lockOnTargetSettings.RotateCameraByPitch ||
		lockOnTargetSettings.RotateCameraByYaw))
	{
		if (!bPauseLock)
		{
		...
		// If has player input
		if (!FMath::IsNearlyEqual(viewRotaitonDelta.Yaw, 0.f, minInputDelta) || !
			FMath::IsNearlyEqual(viewRotaitonDelta.Pitch, 0.f, minInputDelta))
		{
			if (lockOnTargetSettings.bResetTargetAfterPlayerInput)
			{
				...
				{
					GetWorld()->GetTimerManager().SetTimer(ResetAccumulatedPlayerInputTimerHandle,
														   this,
														   &UATPCCameraLockOnTargetObject::ResetAccumulatedPlayerInput,
														   lockOnTargetSettings.
														   ResetAccumulatedPlayerInputForResetTargetDelay,
														   false);
				}
			}

			if (lockOnTargetSettings.bPauseLockAfterPlayerInput && lockOnTargetSettings.PauseLockAfterPlayerInputTime > 0.f
				+ KINDA_SMALL_NUMBER)
			{
				PauseLock();
				GetWorld()->GetTimerManager().SetTimer(PauseLockTimerHandle,
													   this,
													   &UATPCCameraLockOnTargetObject::ResumeLock,
													   lockOnTargetSettings.PauseLockAfterPlayerInputTime,
													   false);
			}
		}
		else
		{
			if (lockOnTargetSettings.bResetTargetAfterPlayerInput && lockOnTargetSettings.
				ResetAccumulatedPlayerInputForResetTargetDelay <= 0.f + KINDA_SMALL_NUMBER)
			{
		...
		if (lockOnTargetSettings.bResetTargetAfterPlayerInput && AccumulatedPlayerInput.IsSet()
			&& (FMath::Abs(AccumulatedPlayerInput->Yaw) >
				FMath::Abs(lockOnTargetSettings.RequiredPlayerInputForResetTarget.Yaw) ||
				FMath::Abs(AccumulatedPlayerInput->Pitch) >
				FMath::Abs(lockOnTargetSettings.RequiredPlayerInputForResetTarget.Pitch)))
		{
...
float UATPCCameraLockOnTargetObject::GetDistanceSquareToActor(AActor* Actor) const
{
	return Actor != nullptr
			   ? FVector::DistSquared(GetTargetLocation(Actor), GetCamera().GetOwner()->GetActorLocation())
			   : 0.f;
}
...
bool UATPCCameraLockOnTargetObject::IsValidDistanceToActor(AActor* Actor) const
{
	...
	{
		auto& lockOnTargetSettings = GetLockOnTargetSettings();
		return !lockOnTargetSettings.bUseMaxLockDistance || GetDistanceSquareToActor(Actor) <= lockOnTargetSettings.
				MaxLockDistance * lockOnTargetSettings.MaxLockDistance;
	}
	return false;
}
...
bool UATPCCameraLockOnTargetObject::IsActorVisible(AActor* Actor) const
{
	if (Actor != nullptr)
	{
		...
		if (lockOnTargetSettings.bUseCheckTargetVisibility)
		{
			...
			UKismetSystemLibrary::LineTraceSingle(Actor,
												  GetCamera().GetCameraLocation(),
												  GetTargetLocation(Actor),
												  UCollisionProfile::Get()->
												  ConvertToTraceType(lockOnTargetSettings.CheckVisibilityTraceChannel),
												  false,
...
{
	return Actor != nullptr
			   ? FRotationMatrix::MakeFromX(GetTargetLocation(Actor) - GetOwningActor()->GetActorLocation()).Rotator()
...
void UATPCCameraLockOnTargetObject::RotateCameraToActor(AActor* Actor, float DeltaSeconds)
{
	...
	if (auto locationObject = camera.GetCameraLocationObject())
	{
		...
		if (lockOnTargetSettings.bUseDesiredConeRotation)
		{
			...
			{
				targetRotation.Yaw = UKismetMathLibrary::ComposeRotators(targetRotation,
																		 FRotator(0.f,
																					  lockOnTargetSettings.
																					  DeisredConeRotationYaw * (
																						  deltaCurrentViewToTarget.Yaw > 0
																							  ? 1
																							  : -1),
																					  0.f)).Yaw;
			}
		...
		auto resultRotation = UATPCFunctionLibrary::InterpRotator(currentViewRotation,
																  targetRotation,
																  DeltaSeconds,
																  lockOnTargetSettings.CameraRotationInterpolation,
																  false);
...
bool UATPCCameraLockOnTargetObject::InternalSetTargetActor(AActor*                 NewTargetActor,
...
void UATPCCameraLockOnTargetObject::PrintDebug()
{
	...
	if (TargetActor.IsValid())
	{
		GEngine->AddOnScreenDebugMessage(-1,
										 0.f,
										 color,
										 FString::Printf(TEXT("Interp Rotation Speed: %s"),
														 *GetLockOnTargetSettings().CameraRotationInterpolation.
														 ToString()));
		GEngine->AddOnScreenDebugMessage(-1,
										 0.f,
										 color,
										 FString::Printf(TEXT("Is paused lock: %s. Remaining pause time: %.2f"),
														 *UKismetStringLibrary::Conv_BoolToString(bPauseLock),
														 GetWorld()->GetTimerManager().
																	 GetTimerRemaining(PauseLockTimerHandle)));
		GEngine->AddOnScreenDebugMessage(-1,
										 0.f,
										 color,
										 FString::Printf(TEXT("Reset accumulated player input through: %.2f"),
														 GetWorld()->GetTimerManager().
																	 GetTimerRemaining(ResetAccumulatedPlayerInputTimerHandle)));
		GEngine->AddOnScreenDebugMessage(-1,
										 0.f,
										 color,
										 FString::Printf(TEXT("Accumulated player input(for bResetTargetAfterPlayerInput): %s"),
														 AccumulatedPlayerInput.IsSet()
															 ? *AccumulatedPlayerInput->ToString()
															 : *FRotator::ZeroRotator.ToString()));
		GEngine->AddOnScreenDebugMessage(-1,
										 0.f,
										 color,
										 FString::Printf(TEXT("Distance to target actor: %.2f"),
														 FMath::Sqrt(GetDistanceSquareToActor(TargetActor.Get()))));
		GEngine->AddOnScreenDebugMessage(-1,
										 0.f,
										 color,
										 FString::Printf(TEXT("Target actor: %s"), *TargetActor->GetName()));
		...
		if (lockOnTargetSettings.bUseDesiredConeRotation)
		{
			...
			const FVector leftLineEnd = lineStart + FRotator(0.f,
															 rotationToTargetYaw + lockOnTargetSettings.
															 DeisredConeRotationYaw,
															 0.f).Vector() * coneDist;
			::DrawDebugLine(GetWorld(), lineStart, leftLineEnd, coneColor, false, -1.f, 0, lineThickness);

			const FVector rightLineEnd = lineStart + FRotator(0.f,
															  rotationToTargetYaw - lockOnTargetSettings.
															  DeisredConeRotationYaw,
															  0.f).Vector() * coneDist;
